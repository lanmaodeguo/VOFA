...
void  HAL_UART_RxCpltCallback(UART_HandleTypeDef  *huart)//串口中断回调函数
{
	if(huart ==&huart1)//判断中断来源（串口1：USB转串口）
    {
       ...
       if((USART1_RX_STA&0x8000)==0){//查询状态位置第十五位，不为1则代表接收未完成继续下流程
           if(USART1_RX_STA&0x4000){//查询状态位第十四位，为1代表接收到了0x0d回车继续下流程 //0x0a换行
               if(USART1_NewData!=0x0a)USART1_RX_STA=0;//接收错误,重新开始
               else USART1_RX_STA|=0x8000;   //接收完成了,给15位置1
           }else{ //查询状态为第十四位 为0还没收到0X0D
               if(USART1_NewData==0x0d)USART1_RX_STA|=0x4000;//接受到回车，第十四位置1
               else{
